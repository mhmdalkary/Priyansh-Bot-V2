...
module.exports.onLoad = async() => {
    ...
    const pathImg = resolve(__dirname, 'cache/canvas', 'marriedv02.png');
    if (!existsSync(dirMaterial)) mkdirSync(dirMaterial, { recursive: true });
    if (!existsSync(pathImg)) await downloadFile("https://i.ibb.co/mc9KNm1/1619885987-21-pibig-info-p-anime-romantika-svadba-anime-krasivo-24.jpg", pathImg);
}
...
async function makeImage({ one, two }) {
    ...
    const batgiam_img = await jimp.read(__root + "/marriedv02.png");
...
module.exports.run = async function ({ event, api, args, usersData }) {    
    ...
    // العشوائي
        ...
        if (!two) return api.sendMessage("😿 للأسف ما لقيت شريك مناسب الحين", threadID, messageID);
    }
    ...
    return api.sendMessage({
        body: `💍 تم توثيق العلاقة مع ${tag} 💖\n🔮 نسبة الانسجام بينكم: ${lovePercent}%`,
